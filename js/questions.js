...
//        $("#labelQuestionText").css("color", "green");
    ...
    function getAnswerValue(question) {
        switch (question['type']) {
            case TYPE_MULTIPLECHOICE:
                ...
                break;
            case TYPE_FREETEXT:
                if($("#inputText").val().trim().length === 0) return -1;
    ...
    function leap_action(contaminated, too_low, hand_detected, height, selected, selected_final,palmposition) {
        ...
        if(selected_radiooption > current_numoptions) selected_radiooption = current_numoptions;
        if(hand_detected) {
    ...
    function setRadioButton(controlname, text) {
        ...
        $(controlname).html("<input type=\"radio\" id=\"radio" + controlname.substr(1) + "\" name=\"1\">" + text);
    }
    ...
    function displayQuestion() {
        if (questions == null || current_question >= questions.length) {
            showMessage("Cannot display question.", "Question " + i + " cannot be displayed");
            return;
        }
        var question = questions[current_question];
